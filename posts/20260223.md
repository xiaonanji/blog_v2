...
- 你会看到`Auto-compaction complete`的log记录
- 原来的输入信息会结合压缩后的上下文再次被提交

手动压缩需要用户使用`/compact`命令触发：
...
当上下文逼近窗口大小时，Clawdbot会自动分析历史上下文，将重要的内容写入`memory/YYYY-MM-DD.md`日志，之后再进行记忆压缩。这样就避免丢失重要的记忆点。

内存刷新可以在`clawdbot.yaml`或`clawdbot.json`文件中配置。
...
剪枝是针对某些工具返回结果太大而设计的。有时候单一`exec`命令可能返回50K字符的log。剪枝技术可以裁剪掉老的工具返回结果。这是一个信息损失的过程：老的工具返回结果一旦被残剪掉就无法恢复了。
...
- 每天重置：每天固定时刻重置对话，比如凌晨4点
- 静默重置：如果这个对话超过N分钟没有任何新的交流则重置
- 每天+静默：两者配合，看谁先发生

### 对话记忆钩子(Session Memory Hook)
当你使用`/new`开始一个新的对话是，对话记忆钩子回自动保存上下文，算法如下：
